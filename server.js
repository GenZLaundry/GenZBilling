...
app.use(cors({
  ...
  allowedHeaders: [
    ...
    'Authorization', 
    'X-Requested-With',
    'x-device-fingerprint',  // Allow the custom header
    'Accept',
    'Origin'
  ]
}));
...
.then(() => {
  ...
  // Auto-initialize upiconfigs collection if not present
  setTimeout(() => {
    if (upiConfigRoutes.initUPIConfigDB) {
      upiConfigRoutes.initUPIConfigDB().catch(err => console.error('⚠️ UPI auto-init error:', err));
    }
  }, 2000);
