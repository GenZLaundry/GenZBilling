...
router.get('/', async (req, res) => {
  try {
    ...
    if (search) {
      ...
      if (/^\+?\d+$/.test(search)) {
        // If searching a phone number, match digits (ignoring leading plus if present)
        const phoneRegexStr = safeSearch.replace(/^\+/, '');
        query.$or = [
          { customerPhone: { $regex: phoneRegexStr } },
          { billNumber: { $regex: safeSearch, $options: 'i' } }
        ];
      } else if (search.toLowerCase().startsWith('gz')) {
        // If searching a bill number explicitly (allow prefix match or suffix match if a short tag number is searched e.g. GZ021785)
        const digitsOnly = safeSearch.slice(2);
        query.$or = [
          { billNumber: { $regex: `^${safeSearch}`, $options: 'i' } },
          ...(digitsOnly ? [{ billNumber: { $regex: `${digitsOnly}$`, $options: 'i' } }] : [])
        ];
      } else {
...
router.post('/', async (req, res) => {
  try {
    ...
    if (existing) {
      // Check if it's the exact same bill (idempotency check)
      const isSameCustomer = existing.customerName === billData.customerName || existing.customerPhone === billData.customerPhone;
      const isSameTotal = Math.round(existing.grandTotal) === Math.round(billData.grandTotal);
      
      if (isSameCustomer && isSameTotal) {
        // Return the existing bill as success (idempotent retry)
        ...
        // Conflict! Same bill number but different customer/amount (collision)
        return res.status(409).json({
          success: false,
          message: 'Bill number collision detected. A different bill already exists with this number.',
          error: 'Bill number collision'
        });
      }
    }
